...
namespace BluePointLilac.Methods
{
    public static class ResourceIcon
    {
        ...
        {
            var flags = FileInfoFlags.SHGFI_ICON | FileInfoFlags.SHGFI_LARGEICON | FileInfoFlags.SHGFI_USEFILEATTRIBUTES;
        ...
        {
            var flags = FileInfoFlags.SHGFI_ICON | FileInfoFlags.SHGFI_LARGEICON;
        ...
        public static Icon GetIcon(string filePath, FileInfoFlags flags)
        {
            ...
            var hInfo = SHGetFileInfo(filePath, 0, ref info, (uint)Marshal.SizeOf(info), flags);
        ...
        public static Icon GetIcon(string iconLocation, out string iconPath, out int iconIndex)
        {
            ...
            else
            {
                ...
                {
                    var flag = int.TryParse(iconLocation[(index + 1)..], out iconIndex);
                    iconPath = flag ? iconLocation[..index] : null;
                }
            }
        ...
        {
            var iconLocation = Registry.GetValue(ShellIconPath, iconIndex.ToString(), null)?.ToString();
